  ...
  try {
    ...
    if (product.reviews && product.reviews.length > 0) {
      ...
      const queryReviews = product.reviews.length
        ...
        : {};
      reviews = (await reviewsCollection.find(queryReviews).toArray()) as Review[];
    }

    const alreadyReviewed = reviews.some(
      (review) => new ObjectId(review.user).toString() === new ObjectId(userId).toString()
    );

    const reviewsWithoutUser = reviews.map((review) => {
